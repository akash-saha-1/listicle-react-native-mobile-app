...
const Input = ({label, type, placeholder, isPassword, value, onChangeText, style, options, ...props}) => {
    ...
    <View style={styles.container}>
        ...
        {type === 'picker' ? (
            <Pressable style={styles.inputContainer} onPress={()=> setIsPickerModalVisible(true)}>
                ...
                <Image style={styles.arrow}  source={require('./../assets/down-arrow.png')} />
            </Pressable>
        ) : (
                ...
                <TouchableOpacity activeOpacity={1} style={styles.modalContent}>
                    ...
                    {options?.map((opt) => {
                        if(opt?.id) {
                            ...
                            return (<Text onPress={()=> {onChangeText(opt); setIsPickerModalVisible(false)}} style={[styles.optionsText, selected ? styles.selectedOptions: {}]} key={opt?.id}>{opt?.title}</Text>)
                        }
                    })}
                </TouchableOpacity>
            </TouchableOpacity>
