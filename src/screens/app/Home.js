...
const Home = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    if (selectedCategory && !keyword) {
      tempFilteredProducts = services?.filter(
        product => String(product.category) === String(selectedCategory),
      );
    } else if (!selectedCategory && keyword) {
      tempFilteredProducts = services?.filter(product =>
    ...
    setFilteredProducts(
      tempFilteredProducts?.filter(
        product => product?.image?.path && product?.title && product?.price,
      ),
    );
